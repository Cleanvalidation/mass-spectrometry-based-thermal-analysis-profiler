...
read_cetsa <- function(protein_path,peptide_path,Prot_Pattern,Peptide=FALSE,Batch=TRUE){
  ...
  if (isTRUE(Peptide)){
    ...
    if(any(names(df.raw)=="Spectrum_File")){
      ...
      df2<-furrr::future_map(df2,.f=function(x) x %>%  
                               ...
                               dplyr::select(-id,Accession,Spectrum_File,AnnotatedSequence,temp_ref,value,Modifications,Charge,"Average_Reporter_S/N",XCorr,"Isolation_Interference_[%]","MissedCleavages","Percolator_PEP","Ion_Inject_Time_[ms]","DeltaM"))
      ...
      df3<-melt(data = df2, 
                id.vars = c("Accession","Annotated.Sequence","Percolator_PEP","Charge","MissedCleavages","DeltaM","Modifications","XCorr"),
                measure.vars = names(df2)[grepl( "Abundance" , names( df2 ) )],
                variable.name = "id",
                value.name = "value")
      df2<-df3 %>% dplyr::mutate(temp_ref = stringr::str_extract_all(id,find),
                                 sample_id= stringr::str_extract_all(id,"F[[:digit:]]+")) %>% 
        tidyr::unnest(cols=c("temp_ref","sample_id")) %>% 
        dplyr::select(-id)
      ...
      df2<-df2[!str_detect(names(df2),'value')]
      ...
      df<-list()
      df<-mclapply(g, read_excel, mc.cores = availableCores())
      
      #function to gather 
      gather_PD <- function(x){ x %>%  
          dplyr::select(Accession,tidyselect::starts_with('Abundance'),-tidyselect::contains('Grouped')) %>% 
          tidyr::gather('id', 'value', -Accession) 
      }
      df<-mclapply(df,gather_PD,mc.cores=availableCores())
      
      label_PD<-function(x){x %>%
          dplyr::mutate(sample_id = as.character(ifelse(!is.na(str_extract(id, str_c('F',"[:digit:][:digit:]"))),str_extract(id, str_c('F',"[:digit:][:digit:]")),stringr::str_extract_all(id,str_c('F','[:digit:]')))),
                        temp_ref = stringr::str_extract_all(id,find),
                        missing=ifelse(is.na(value),1,0))}
      
      df<-mclapply(df,label_PD,mc.cores=availableCores())
      
      
      if(any(names(df[[1]])=="Spectrum_File")){
        ...
        df<-dplyr::bind_rows(df) %>% 
          tidyr::unnest(cols = temp_ref) %>%
          dplyr::select(Accession,sample_id,value,temp_ref) %>%
          dplyr::rename(Protein_value=value)
        df2<-df2 %>% dplyr::mutate(sample_id=str_extract(id,"F[[:digit:]]+"))
        
        df2<-data.table(df2)
        df<-data.table(df)
        
        #Use I column to distinguish duplicated rank values/PSM
        col_n<-c("sample_id","Accession")
        #set join columns
        setkeyv(df,cols=col_n)
        setkeyv(df2,cols=col_n)
        #right_join
        df2<- df2 %>% dplyr::group_split(Accession)
        df<-df %>% dplyr::group_split(Accession)
        
        df3 <- furrr::future_map2(df2,df,function(x,y) data.frame(merge(x,y, all.x=TRUE)))
  ...
  CP<-function(df_0,d,PSM=FALSE){#df_0 is the result data frame and d is the orginal data with replicates
    ...
    if(isTRUE(PSM)){
      ...
      dap<-df_0 %>% dplyr::left_join(Split,by=c("C"="C","uniqueID"="uniqueID","dataset"="dataset","I"="I","sample_name"="sample_name","CC"="CC","n"="n","sample"="sample","rank"="rank"))
  ...
  tlCI<-function(i,df1,df2,Df1,overlay=TRUE,residuals=FALSE,df.temps,PSMs){
    ...
    if(isTRUE(PSMs)){
      ...
      PLR_P1<-ggplot2::ggplot(Pred1, ggplot2::aes(x = C,y = fit,color=Treatment))+
        ...
        annotate("text",
                 x = max(Pred1$C-3,na.rm=TRUE),
                 y = max(Pred1$I-0.3,na.rm=TRUE),
      ...
      PLR_P2<-PLR_P1+ggplot2::geom_point(Pred2, mapping=ggplot2::aes(x = C,y = I,color=Treatment)) +
        ...
        annotate("text",
                 x = max(Pred2$C-3,na.rm=TRUE),
                 y = max(Pred2$I-0.5,na.rm=TRUE),
  ...
  df_raw <- read_cetsa("~/Files/Scripts/Files/Reprocessed_March","~/Files/Scripts/Files/Reprocessed_March","_Proteins",Peptide=TRUE,Batch=FALSE)                                                              
  #saveRDS(df_raw,"df_raw.RDS")
  df_raw<-df_raw %>% dplyr::group_split(Spectrum_File)
  ...
  df_raw<-furrr::future_map(df_raw,function(x) try(filter_PSMs(x,20,0.1,1.8,50,1,1,6,15)))
  #new for PSMs
  ...
  df.samples<-df.s(f,dplyr::bind_rows(df_raw),3,2,"DMSO","TREATED",Batch=TRUE,PSM=FALSE)
  
  
  
  #assign TMT channel and temperature data
  df_clean <- furrr::future_map(df_raw,function(x) clean_cetsa(x, temperatures = df.temps, samples = df.samples,Peptide=TRUE,solvent="DMSO",CFS=TRUE))#assgns temperature and replicate values
  ...
  ggplot2::ggplot(dplyr::bind_rows(df_norm),mapping=aes(x=C,y=I3,color=sample_name))+facet_wrap(vars(sample_name,dataset),nrow=4)+
    geom_jitter(position=position_jitter(2),alpha=0.5)+
    geom_boxplot(alpha=0.8)+xlab('Temperature (\u00B0C)')+ylab("Normalized Intensity (Top500 HML)")
  ggplot2::ggplot(dplyr::bind_rows(df_norm),mapping=aes(x=C,y=I5,color=sample_name))+facet_wrap(vars(sample_name,dataset),nrow=4)+
    geom_jitter(position=position_jitter(2),alpha=0.5)+
    geom_boxplot(alpha=0.8)+xlab('Temperature (\u00B0C)')+ylab("Normalized Intensity (Top600 HML)")
  ggplot2::ggplot(dplyr::bind_rows(df_norm),mapping=aes(x=C,y=I10,color=sample_name))+facet_wrap(vars(sample_name,dataset),nrow=4)+
    geom_jitter(position=position_jitter(2),alpha=0.5)+
    geom_boxplot(alpha=0.8)+xlab('Temperature (\u00B0C)')+ylab("Normalized Intensity (Top700 HML)")
  ...
  PlotTrilinear<-function(df_norm,target,df.temps,Ft,filt,Peptide=FALSE){
    ...
    if(isTRUE(Peptide) & any(names(df_norm)=="XCorr")){
      df_norm<-df_norm %>% dplyr::select(-XCorr,-temp_ref,-Modifications,-Missed_Cleavages,-DeltaM,-Annotated_Sequence,-Spectrum_File,-PEP,-Charge)
      if(any(names(df_norm)=="I_Interference")){
        df_norm<-df_norm %>% dplyr::select(-I_Interference,-IonInjTime,-S_N)
      }
      df_norm<-df_norm %>% dplyr::mutate(I=I10)%>% dplyr::select(-I3,-I5,-I10)
    ...
    df_<-suppressWarnings(CP(results,d_,PSM=Peptide))
    df_1<-suppressWarnings(CP(results_t,d_1,PSM=Peptide))
    DFN<-suppressWarnings(CP(results_n,DF,PSM=Peptide)) 
  ...
  plot<-purrr::map(df_norm,function(x) try(PlotTrilinear(x,"Q02750",df.temps,Ft=FALSE,filt=FALSE,Peptide=TRUE)))
  check<-ggplot2::ggplot_build(plot[[1]])
  y<-get_legend(check$plot)
  data<-order(unlist(lapply(plot,function(x) x$labels$title)))
  plot<-plot[data]
  ...
  ###############################
  plot_Splines<-function(x,Protein,df.temps,MD=FALSE,Filters=TRUE,fT=FALSE,show_results=FALSE,PSM=FALSE){
    if(isTRUE(MD)){
