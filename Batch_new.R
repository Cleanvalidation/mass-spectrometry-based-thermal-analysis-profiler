...
clean_cetsa <- function(df, temperatures = NULL, samples = NULL,Peptide=FALSE,solvent,CFS=TRUE,CARRIER=TRUE){
  ...
  if(isTRUE(Peptide) & any(isTRUE(str_detect(names(df),"Fraction")))){#if this is a peptide file and is fractionated
    ...
    df<-purrr::map(df,function(x) x%>%
                     dplyr::mutate(value = value / value[temperature == min(temperatures$temperature,na.rm=TRUE)]) %>% distinct(.) %>% ungroup(.))
    df<-dplyr::bind_rows(df)
    if(any(names(df)=="sample_id" & !any(names(df)=="sample"))){
...
spstat<-function(DF,df,df1,Ftest=TRUE,show_results=TRUE,filters=TRUE,scaled_dof=FALSE,Peptide=FALSE){
  ...
  results_3<-purrr::map(results_3,function(x) x %>% dplyr::mutate(
    dTm=x$Tm[x$dataset %in% "treated"]-x$Tm[x$dataset %in% "vehicle"])%>% dplyr::ungroup(.)) 
...
TPPbenchmark_generic<-function(f,overlaps=NA,volcano=TRUE,filters="TPP",Peptide=FALSE){
  ...
  check1<-df_1 %>% count(sample_name) %>%
    ...
    scale_fill_manual(values = colors$hex,aesthetics="fill")+
    xlim(-1.1,1.45)+
    ggplot2::geom_label(mapping=aes(x=c(1,1.2,1.1,0.7,-0.8,-1,-1.1,-0.75),y=c(1,0.42,-0.38,-0.88,-0.88,-0.38,0.42,1),label=n,color=sample_name),inherit.aes=TRUE,vjust="top",show.legend = FALSE)+
    ggtitle("Number of fitted curves")
...
UpSet_curves<-function(f,Trilinear=FALSE,Splines=TRUE,Sigmoidal=FALSE,Peptide=FALSE,filter=FALSE){
  ...
  if(isTRUE(Peptide) & any(names(f)=="rank_l")){
    f<-dplyr::bind_rows(f) %>% dplyr::mutate(sample_name=as.factor(sample_name),dataset=as.factor(dataset)) %>%
      dplyr::select(-rank,-rank_l,-C,-I,-temp_ref,-CV_pct,-missing_pct) %>% dplyr::filter(!is.na(sample_name)) %>%
...
volcano_data<-function(f,Trilinear=FALSE,Splines=FALSE,Sigmoidal=TRUE,Peptide=FALSE,benchmark=TRUE,Fhist=TRUE,labels=FALSE,type=NA){
  ...
  if(isTRUE(Trilinear)){
    ...
    df_<-f %>% 
      dplyr::select(uniqueID,p_dTm,sample_name,dTm,Tm,rss,AUC,rsq,stabilized,ptest) %>% 
      distinct(.)
    df_$diffexpressed <- "Not Shifted"
    # if log2Foldchange > 0.6 and pvalue < 0.05, set as "UP" 
    ...
    df_$delabel <- NA
    Names<-as.character(f$uniqueID[which(df_$ptest<0.05 & !f$diffexpressed=="Not Shifted")])
...
saveRDS(plotS2,"CFS_Peptide_unfiltered_Consensus_data.RDS")
...
pdf("PEPTIDE_UNFILTERED_NO_IMAATSA.pdf",encoding="CP1253.enc",compress=FALSE,width=12.13,height=7.93)
P2
dev.off()
