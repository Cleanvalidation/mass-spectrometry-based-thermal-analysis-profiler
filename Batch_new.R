...
read_cetsa <- function(protein_path,peptide_path,Prot_Pattern,Peptide=FALSE,Frac=TRUE,CFS=TRUE,solvent="DMSO",CARRIER=TRUE,rank=TRUE,sub=NA,temperatures=temps,baseline="min",NORM="QUANTILE",keep_shared_proteins=FALSE){
  ...
  if(any(stringr::str_detect(Peptide,c("PG","PSMs")))){
    ...
    if (Peptide=="PSMs"){ #if this is a PSM file
      ...
      }else{#if there's no scaling factor implemented, set as 1
        df2<-purrr::map(PSMs,function(x) x %>% dplyr::mutate(scal_fac=1))
      }
      ...
      df2<-dplyr::bind_rows(df2) %>%
        dplyr::group_by(uniqueID,treatment,sample_id,sample_name) %>% 
        dplyr::group_split()
    ...
    Proteins <- Proteins %>% 
      ...
      dplyr::mutate(File.ID = stringr::str_extract(id,"[:upper:][[:digit:]]+"),
                    ...
                    temp_ref=stringr::str_extract(id,"[:digit:][:digit:][:digit:][N|C]|126|131"))
    Proteins<-Proteins %>% dplyr::right_join(temperatures, by ="temp_ref") %>% distinct(.)
    
    
    Proteins<-data.table::data.table(dplyr::bind_rows(Proteins))
    PSMs<-data.table::data.table(dplyr::bind_rows(PSMs)) %>%
      dplyr::mutate(Spectrum.File=stringr::str_remove(Spectrum.File,"[[:digit:]]+.raw")) %>% 
      dplyr::distinct(.)
    ...
    if(isTRUE(rank)){
      ...
      df_raw_D_R<-df_raw_D_R %>% purrr::keep(function(x) nrow(x)>0)
      if(any(names(Proteins)=="Fraction")&any(stringr::str_detect(Proteins[[1]]$Spectrum.File,"Fraction"))){
        Proteins<-dplyr::bind_rows(Proteins)
...
choose_PSM<-function(x,Frac=Frac,NORM=NORM,CARRIER=CARRIER,subset=subset,baseline=baseline,Prot_Pattern=Prot_Pattern,Peptide=Peptide){
  ...
  df2<-df2 %>% dplyr::filter(Accession %in% united1$Accession,Annotated_Sequence %in% united1$Annotated_Sequence)
  mat_norm<-which(stringr::str_detect(names(df2),"Abundance"))
  
  united<-df2 %>% dplyr::ungroup() %>% 
    dplyr::group_by(Accession,sample_name,treatment) %>%
    dplyr::group_split()
  united1<-united
  test<-purrr::map(united,function(y)log2(medianPolish(as.matrix(y[,mat_norm]),ncol(y[,mat_norm]))))
  
  
  if(is.na(NORM)){
    check<-(test)
  }else if(NORM=="QUANTILE"){
    check<-furrr::future_map(test,function(x) {
    ...
    check<-furrr::future_map(test,function(x) tryCatch(DEqMS::equalMedianNormalization(as.matrix(x)),
                                                       error = function(e) {print(e)
                                                         return(NA)},
                                                       finally = {print("Equal median normalization finished")})
    )
  }else if(NORM=="VSN"){
    check<-furrr::future_map(test,function(x) tryCatch(vsn::vsn2(as.matrix(x)),
                                                       error = function(e) {print(e)
                                                         return(NA)},
                                                       finally = {print("Variance stabilizing normalization finished")}))
  }
  
  test1<-purrr::map2(check,united,function(x,y) set_names(x,names(y[,mat_norm])))
  result<-purrr::map2(united,test1,function(x,y){
    x=x %>% dplyr::select(-names(y)) %>%dplyr::distinct()
    x=append(x,y) %>% as.data.frame()
    return(x)
  })
  #transform abundance to log2 abundance
  
  #pivot data to long format after aggregating PSM abundances to protein level
  df2<-dplyr::bind_rows(result) %>%tidyr::pivot_longer(cols=colnames(dplyr::bind_rows(result))[stringr::str_detect(colnames(dplyr::bind_rows(result)),c('[:digit:][:digit:][:digit:][N|C]|126|131'))],
                                                       names_to="temp_ref",
                                                       values_to="value")
  
  #from the PSM subset, keep those accessions where the conditional min # of replicates are met
  if(any(names(dplyr::bind_rows(df2)=="Fraction"))|any(stringr::str_detect(df2$File_ID[1],"."))){
    if(any(any(stringr::str_detect(df2$File_ID[1],".")))){
      df2$Fraction<-stringr::str_remove(df2$File_ID,"F[[:digit:]]+.")
      df2$sample_id<-stringr::str_extract(df2$File_ID,"F[[:digit:]]+")
...
clean_cetsa <- function(df, temperatures = NULL,samples = NA,Peptide=FALSE,solvent,CFS=TRUE,CARRIER=TRUE,baseline="min"){
  ...
  if(any(stringr::str_detect(df$temp_ref[1],"Abundance"))){
    df$temp_ref<-stringr::str_remove(df$temp_ref,"Abundance_")
  }
  df <- df %>% dplyr::ungroup(.)
  if(ncol(temperatures)>2){#If dealing with a time_point study 
    ...
    if(!any(names(df)=="sample_name")&!any(names(df)=="treatment")){
      ...
      df_<-df
      df_$treatment<-ifelse(stringr::str_detect(df_$sample_name,solvent),"vehicle","treated")
  ...
  #add replicates
  df<-replicate_labels(df)
  if(any(names(df)=="File.ID")&isTRUE(Peptide)){
  ...
  if(any(names(df)=="Fraction")&any(names(df)=="File.ID")){
    if(any(stringr::str_detect(df$Fraction,"."))){
      df<-df %>% dplyr::mutate(Fraction = stringr::str_remove(File.ID,"[:upper:][[:digit:]]+."))
    }
  }
...
cetsa_fit <- function(d, norm = FALSE) {
  ...
  result = tryCatch({
    ...
    #c(Pl=0, a = 550, b = 10
    fine_start <- expand.grid(p=c(0,0.2),k=seq(200,500),m=seq(45,75,by=5))
...
cetsa_fit2 <- function(d, norm = FALSE) {
  ...
  result = tryCatch({
    ...
    #c(Pl=0, a = 550, b = 10
    fine_start <- expand.grid(p=c(0,0.2),k=seq(200,1000),m=seq(45,75,by=5))
    new_start <- nls2::nls2(y ~ fit.cetsa(p, k, m, t),
                            data = myData,
                            start = fine_start,
                            algorithm = "grid-search",#note: check other ones
...
spstat<-function(DF,df,df1,Ftest=TRUE,show_results=TRUE,filters=TRUE,scaled_dof=FALSE,Peptide=FALSE){
  ...
  if(any(names(m[[1]])=="Annotated_Sequence")&any(names(m[[1]])=="replicate")){
    m<-dplyr::bind_rows(m) %>%
      dplyr::group_split(uniqueID,Annotated_Sequence,treatment,sample_id,replicate)
  }else if(any(names(m[[1]])=="uniqueID")&any(names(m[[1]])=="replicate")){
    m<-dplyr::bind_rows(m) %>%
      dplyr::group_split(uniqueID,treatment,sample_id,replicate)
  }else if (any(names(m[[1]])=="Annotated_Sequence")&any(names(m[[1]])=="Fraction")){
    m<-dplyr::bind_rows(m) %>%
      dplyr::group_split(uniqueID,Annotated_Sequence,treatment,sample_id,Fraction)
  }else if (any(names(m[[1]])=="uniqueID")&any(names(m[[1]])=="Fraction")){
    m<-dplyr::bind_rows(m) %>%
      dplyr::group_split(uniqueID,treatment,sample_id,Fraction)
  }
  ...
  if(any(names(m1[[1]])=="Annotated_Sequence")&any(names(m1[[1]])=="replicate")){
    m1<-dplyr::bind_rows(m1) %>%
      dplyr::group_split(uniqueID,Annotated_Sequence,treatment,sample_id,replicate)
  }else if(any(names(m1[[1]])=="uniqueID")&any(names(m1[[1]])=="replicate")){
    m1<-dplyr::bind_rows(m1) %>%
      dplyr::group_split(uniqueID,treatment,sample_id,replicate)
  }else if (any(names(m1[[1]])=="Annotated_Sequence")&any(names(m1[[1]])=="Fraction")){
    m1<-dplyr::bind_rows(m1) %>%
      dplyr::group_split(uniqueID,Annotated_Sequence,treatment,sample_id,Fraction)
  }else if (any(names(m1[[1]])=="uniqueID")&any(names(m1[[1]])=="Fraction")){
    m1<-dplyr::bind_rows(m1) %>%
      dplyr::group_split(uniqueID,treatment,sample_id,Fraction)
  }
  ...
  if(length(mn)==0){
    ...
    if(any(names(mn[[1]])=="Annotated_Sequence")&any(names(mn[[1]])=="replicate")){
      mn<-dplyr::bind_rows(mn) %>%
        dplyr::group_split(uniqueID,Annotated_Sequence,treatment,sample_id,replicate)
    }else if(any(names(mn[[1]])=="uniqueID")&any(names(mn[[1]])=="replicate")){
      mn<-dplyr::bind_rows(mn) %>%
        dplyr::group_split(uniqueID,treatment,sample_id,replicate)
    }else if (any(names(mn[[1]])=="Annotated_Sequence")&any(names(mn[[1]])=="Fraction")){
      mn<-dplyr::bind_rows(mn) %>%
        dplyr::group_split(uniqueID,Annotated_Sequence,treatment,sample_id,Fraction)
    }else if (any(names(mn[[1]])=="uniqueID")&any(names(mn[[1]])=="Fraction")){
      mn<-dplyr::bind_rows(mn) %>%
        dplyr::group_split(uniqueID,treatment,sample_id,Fraction)
    }
  ...
  #combine vehicle and treated data to calculate dTm values
  
  if(isTRUE(Peptide)){
    results<-dplyr::bind_rows(mean1,mean1_1) %>% dplyr::filter(!is.na(Tm))
    results_<-dplyr::bind_rows(results) %>%
      dplyr::select(uniqueID,Annotated_Sequence,sample_name,sample_id,treatment,C,I,Tm) %>%
      dplyr::group_by(uniqueID,sample_name) %>% 
      distinct(.) %>% dplyr::group_split()
    results_<-results_ %>% purrr::keep(function(x) nrow(x)>1&length(unique(x$sample_id))>3)
    results_<-purrr::map(results_,function(x) x %>% dplyr::mutate(dTm=(mean(x$Tm[x$treatment=="treated"],na.rm=TRUE)-mean(x$Tm[x$treatment=="vehicle"],na.rm=TRUE))))
    
  }else if(!isTRUE(Peptide)){
    results<-dplyr::bind_rows(mean1,mean1_1)  %>% dplyr::filter(!is.na(Tm))
    results_<-dplyr::bind_rows(results) %>%
      dplyr::select(uniqueID,sample_name,sample_id,treatment,C,I,Tm) %>%
      dplyr::group_by(uniqueID,sample_name) %>% 
      distinct(.) %>% dplyr::group_split()
    results_<-results_ %>% purrr::keep(function(x) nrow(x)>1&length(unique(x$sample_id))>3)
    results_<-purrr::map(results_,function(x) x %>% dplyr::mutate(dTm=(mean(.$Tm[.$treatment=="treated"],na.rm=TRUE)-mean(.$Tm[.$treatment=="vehicle"],na.rm=TRUE))))
    
  }
  mean3<-dplyr::bind_rows(mean3) %>% dplyr::mutate(dTm=NA)
  ...
  nesting<-names(results_2[[1]])[!names(results_2[[1]]) %in% c("uniqueID","sample_id","treatment","Tm","replicate","sample_name","dTm")]
  ...
  results_2<-dplyr::bind_rows(results_2) %>%
    dplyr::group_split(uniqueID)
  if(any(names(results_2[[1]])=="replicate")){
    #check data for duplicate replicate values and recalculate dTm after using distinct(.)
...
spf<-function(spresults,DFN,filters = TRUE){
  if(any(names(spresults)=="replicate")){
    spresults<-spresults %>% 
...
spCI<-function(i,df1,df2,Df1,df.temps,overlay=TRUE,alpha,residuals=FALSE,simulations=FALSE,CI=TRUE,Peptide=FALSE,CARRIER=TRUE,Protein=Protein,raw=FALSE){
  ...
  if(isTRUE(Peptide)){
    Df1<-dplyr::bind_rows(Df1) %>%
      dplyr::group_by(uniqueID,Annotated_Sequence,treatment,C,sample_name) %>% 
      dplyr::group_split()
    Df1<-purrr::map(Df1,function(x)x %>% dplyr::mutate(Replicate=row.names(x)))
    
    df2<-dplyr::bind_rows(df2) %>%
      dplyr::group_by(uniqueID,Annotated_Sequence,treatment,C,sample_name) %>% 
      dplyr::group_split()
    df2<-purrr::map(df2,function(x)x %>% dplyr::mutate(Replicate=row.names(x)))
    if(any(names(Df1)=="replicate")){
      Df1<-purrr::map(Df1,function(x) x %>% dplyr::mutate(replicate=Replicate))
      df2<-purrr::map(df2,function(x) x %>% dplyr::mutate(replicate=Replicate))
    }else{
      Df1<-dplyr::bind_rows(Df1) %>%
        dplyr::group_by(uniqueID,treatment,C,sample_name) %>% 
        dplyr::group_split()
      Df1<-purrr::map(Df1,function(x)x %>% dplyr::mutate(Replicate=row.names(x)))
      ...
      df2<-dplyr::bind_rows(df2) %>%
        ...
        dplyr::group_split()
      df2<-purrr::map(df2,function(x)x %>% dplyr::mutate(Replicate=row.names(x)))
      if(any(names(Df1[[1]])=="replicate")){
        Df1<-purrr::map(Df1,function(x) x %>% dplyr::mutate(replicate=Replicate))
        df2<-purrr::map(df2,function(x) x %>% dplyr::mutate(replicate=Replicate))
      }
    }
    df2<-dplyr::bind_rows(df2)
    df2<-df2[,!stringr::str_detect(names(df2),"File.ID|Channel|RT|Confidence|Protein|Percolator|DeltaM|Tm|rsq|CC|k_|AUC")]
  ...
  null<-Df1[which(Df1$uniqueID %in% df1),]
  if(nrow(null)==0){
  ...
  plot<-ggplot(pred,mapping= ggplot2::aes(x = temperature,y=fit,color=treatment))+
    geom_point(BSvar, mapping=ggplot2::aes(x=temperature,y=I,color = treatment))+
  ...
  if(isTRUE(CI)){
    ...
    if(isTRUE(Peptide)){
      if(!any(names(BSVar)=="Fraction")){#if there is no replicate column
        BSVar<-dplyr::bind_rows(BSVar) %>%
          dplyr::select(uniqueID,treatment,sample_name,C,I) %>% 
          distinct(.) %>%
          dplyr::group_by(uniqueID,treatment,sample_name,C,I) %>%
          dplyr::group_split()
        BSVar<-lapply(BSVar,function(x) x %>% dplyr::mutate(Replicate=as.factor(row.names(x))))
        ...
        BSVar1<-dplyr::bind_rows(BSVar1) %>%
          ...
          dplyr::group_by(uniqueID,treatment,sample_name,C,I) %>%
          dplyr::group_split()
        BSVar1<-lapply(BSVar1,function(x) x %>% dplyr::mutate(Replicate=as.factor(row.names(x))))
        BSVar1<-dplyr::bind_rows(BSVar1) %>% dplyr::ungroup()
        
      }else if(any(names(BSVar)=="Fraction")){#if there is no replicate column
        BSVar<-dplyr::bind_rows(BSVar) %>%
          dplyr::select(uniqueID,treatment,sample_name,Fraction,C,I) %>% 
          distinct(.) %>%
          dplyr::group_by(uniqueID,treatment,sample_name,Fraction,C,I) %>%
          dplyr::group_split()
        BSVar<-lapply(BSVar,function(x) x %>% dplyr::mutate(Replicate=as.factor(row.names(x))))
        BSVar<-dplyr::bind_rows(BSVar) %>% dplyr::ungroup()
      ...
      if(!isTRUE(raw)&!all(BSVar$Stroke)==0){#if I isnt raw and there's no rank column for the data
        plot1<-ggplot2::ggplot(BSVar,ggplot2::aes(x =C,y = I,color=treatment))+
          ggplot2::geom_point(BSVar,mapping=ggplot2::aes(x=C,y=I,color = treatment,shape=Replicate))+
        ...
        plot1<-ggplot2::ggplot(BSVar,ggplot2::aes(x =C,y = I,color=treatment,shape=Replicate))+
          geom_point(data=BSVar[BSVar$Stroke==1,],
      ...
      if(any(names(BSVar)=="rank_l")){#if this protein file has a rank column
        ...
        plot<-plot1+
          ...
          annotate("segment", x = min(fitted.values1$C,na.rm=TRUE), xend = signif(with(fitted.values1, stats::approx(fitted.values1$fit,fitted.values1$C,xout=max(fitted.values1$fit, na.rm=TRUE)-0.5))$y,3),
                   y = 0.5, yend = 0.5,
                   colour = "red",linetype=2)+
          ...
          annotate("segment", x = min(fitted.values$C,na.rm=TRUE), xend = signif(with(fitted.values, stats::approx(fitted.values$fit,fitted.values$C,xout=max(fitted.values$fit, na.rm=TRUE)-0.5))$y,3),
                   y = 0.5, yend = 0.5,
                   colour = "blue",linetype=2)+
          annotate("segment", x = signif(with(fitted.values, stats::approx(fitted.values$fit,fitted.values$C,xout=max(fitted.values$fit, na.rm=TRUE)-0.5))$y,3),
                   xend = signif(with(fitted.values, stats::approx(fitted.values$fit,fitted.values$C,xout=max(fitted.values$fit, na.rm=TRUE)-0.5))$y,3), y = 0, yend = 0.5,
...
MSStats_converter<-function(df_raw,solvent="DMSO",ref="126",Frac=TRUE,CFS=FALSE,Peptide=FALSE){
  ...
  if(any(stringr::str_detect(editPSMs2$id,"_[:digit:]"))){
    editPSMs2$replicate<-stringr::str_extract(editPSMs2$id,",_[:digit:]")
    editPSMs2$replicate<-stringr::str_extract(editPSMs2$replicate,"[:digit:]")
    editPSMs2$TechRepMixture<-editPSMs2$replicate
  }else if(any(stringr::str_detect(editPSMs2$id,".[:digit:]"))){
    editPSMs2$replicate<-stringr::str_extract_all(editPSMs2$id,".[:digit:]")
    editPSMs2$replicate<-purrr::map(editPSMs2$replicate,function(x)str_c(x,collapse=""))
    editPSMs2$replicate<-purrr::map(editPSMs2$replicate,function(x)as.factor(str_trunc(x,1,side=c("left"),ellipsis="")))
    editPSMs2$replicate<-unlist(editPSMs2$replicate)
    editPSMs2<-editPSMs2[!is.na(editPSMs2$replicate),]
    editPSMs2$TechRepMixture<-editPSMs2$replicate
  }
  ...
  if(any(names(df_raw)=="Annotated_Sequence")){
    editPSMs2<-editPSMs2 %>%
      dplyr::rename("ProteinName"="Master_Protein_Accessions",
                    ...
                    "Channel"="temp_ref",
                    "Intensity"="value") %>% 
      dplyr::mutate(IsotopeLabelType="L",
  ...
  if(!any(names(editPSMs2)=="Annotated_Sequence")){
    ...
    Protein<-editPSMs2 %>% dplyr::mutate(Master_Protein_Accessions=Protein.Accessions,
                                         File_ID=as.factor(stringr::str_extract(id,"F[[:digit:]]+")))
    
    JoinPSM_Prot<-Protein %>% dplyr::right_join(PSMs)
    JoinPSM_Prot<-JoinPSM_Prot[!is.na(JoinPSM_Prot$TechRepMixture),]
...
rename_TPP<-function(x,temps=df.temps,string=FALSE){#rename script data to run TPP
  ...
  TPP_Cliff$replicate<-stringr::str_extract(TPP_Cliff$Spectrum.File,"[[:upper:]][[:digit:]]+_|[[:lower:]][[:digit:]]+_")
  TPP_Cliff$replicate<-stringr::str_extract(TPP_Cliff$replicate,"[[:digit:]]+")
  ...
  #TPP_Cliff<-purrr::map(TPP_Cliff,function(x) x %>% dplyr::filter(!is.na(I)))
  ...
  TPP_Cliff<-dplyr::bind_rows(TPP_Cliff) %>% dplyr::select(gene_name,Spectrum.File,sample_id,replicate,Condition,tidyr::starts_with("rel"),I,temp_ref) %>% dplyr::filter(!is.na(I))
  ...
  if(isTRUE(NORM)){
    TPP_Cliff<-just_TPP_norm(TPP_Cliff)
    return(TPP_Cliff)
  }
  return(TPP_Cliff)
}
just_TPP_norm<-function(TPP_Cliff,df.temps){
  reference<-TPP_Cliff
  TPP_Cliff<-dplyr::bind_rows(TPP_Cliff) %>% dplyr::select(-dplyr::starts_with("rel_fc_131H"))
  ...
  test<-dplyr::bind_rows(TPP_Cliff)%>%
    dplyr::group_by(gene_name,Condition,replicate) %>%
    dplyr::mutate(across(starts_with("rel_fc"),.fn=function(x) x/rel_fc_126))
  test_Cliff<-dplyr::bind_rows(test)
  test_Cliff<-test_Cliff %>% dplyr::ungroup() %>% group_by(Experiment) %>% dplyr::group_split()
  
  names(test_Cliff)<-as.vector(lapply(test_Cliff,function(x) unique(x$Experiment)))
  
  test_Cliff<-lapply(test_Cliff,function(x)as.data.frame(x))
  
  
  imp<-tryCatch(TPP::tpptrImport(configTable = config,data=test_Cliff),
                error=function(cond){
                  config<-config[c(3,2,1,4),]
                  y<-TPP::tpptrImport(configTable = config,data=test_Cliff)
                  return(y)
                })
  normTPP<-TPP::tpptrNormalize(imp,normReqs = TRreqs)
  #get data back to msstats format 
  vault<-purrr::map(normTPP$normData,function(x) Biobase::exprs(x))
  ...
  col_info<-dplyr::bind_rows(TPP_Cliff) %>%
    tidyr::pivot_longer(cols=names(TPP_Cliff)[stringr::str_detect(names(TPP_Cliff),"rel_fc")],
                        names_to="id",
                        values_to="Abundance") %>% 
    dplyr::select(Experiment,Condition,sample_id,Spectrum.File) %>% 
    dplyr::mutate(Condition=ifelse(Condition=="Treatment","treated","vehicle")) %>% 
    distinct(.)
...
runTPP<-function(x,df.temps){
  ...
  trData <- tpptrImport(configTable = config, data = check)
...
filter_Peptides<-function(df_,S_N,PEP,XCor,Is_Int,Missed_C,Mods,Charg,DeltaMppm,Occupancy,filter_rank=FALSE,keep_shared_proteins=FALSE,CFS=TRUE,Frac=FALSE){
  ...
  if(!any(names(df_)=="sample_name")&any(names(df_)=="Spectrum.File")){
  ...
  df_<-df_ %>%
    dplyr::filter("AR_S_N">S_N,P_PEP<PEP,Charge<Charg,MissedCleavages<Missed_C,abs(DeltaM)<DeltaMppm)
...
read_xl<-function(x) {
  ...
  if (any(names(df2)=="Master.Protein.Accessions")&!any(names(df2)=="Accession")){
    ...
    df2<-df2 %>% dplyr::rename("Accession"=names(df2)[stringr::str_detect(names(df2),"Accession")][1])
  }
...
read_PD<-function(x,CFS=FALSE){#rename data for peptide groups
...
df_raw <- read_cetsa("E:/Covid/Fractions/PD_Files",
...
plot_Splines<-function(x,Protein="Q02750",df.temps,Filters=FALSE,fT=TRUE,show_results=TRUE,Peptide=TRUE,CI=TRUE,simulations=FALSE,CARRIER=FALSE,Frac=TRUE,raw=FALSE){
  ...
  if(any(names(x)=="temperature")&!any(names(x)=="C")){
    x<-x %>% dplyr::rename("C"="temperature")
  }
  if(any(names(x)=="value")&!any(names(x)=="I")){
    x<-x %>% dplyr::rename("I"="value")
  ...
  if(any(names(x)=="Accession")&!any(names(x)=="uniqueID")){
    x<-x %>% dplyr::mutate(uniqueID=Accession)
  ...
  if(any(names(x)=="Annotated_Sequence")){
    x<-dplyr::bind_rows(x) 
    distinct(.) %>% 
      dplyr::group_by(uniqueID,Annotated_Sequence,temp_ref,treatment,sample_name) %>% 
      dplyr::group_split()
    x<-purrr::map(x,function(x)x %>% dplyr::mutate(Replicate=row.names(.),
                                                   replicate=row.names(.)))
  }else if(any(names(x)=="Annotated_Sequence")){
    x<-dplyr::bind_rows(x) %>%
      distinct(.) %>% 
      dplyr::group_by(uniqueID,Annotated_Sequence,temp_ref,treatment,sample_name) %>% 
      dplyr::group_split()
...
plotS2 <- purrr::map(df_norm,function(x) try(plot_Splines(x,"F1Q7F3",df.temps,Filters=FALSE,fT=FALSE,show_results=FALSE,Peptide=FALSE,simulations=FALSE,CARRIER=TRUE,Frac=FALSE,raw=FALSE)))
P1<-plotS2
...
plotS2 <- purrr::map(df_clean_ch,function(x) try(plot_Splines(x,"F1Q7F3",df.temps,MD=TRUE,Filters=FALSE,fT=FALSE,show_results=FALSE,Peptide=TRUE,simulations=FALSE,CARRIER=TRUE,Frac=TRUE,raw=TRUE)))
check<-ggplot2::ggplot_build(plotS2[[1]])
y<-get_legend(check$plot)
# data<-unlist(lapply(plotS2,function(x) x$labels$title))
# plotS2<-plotS2[order(data)]
