...
d<-df_norm %>% unique(.) %>% ungroup(.)#save unique data 
...
Dsum$Tma<-Dsum1$Tma
Dsum<-Dsum %>% dplyr::mutate(rank = ntile(Dsum$Tma,7))
Dsum<-arrange(Dsum, desc(Tma), desc(RSSd))  %>% dplyr::filter(RSSd>0) 
...
sigCI <- function(object, parm, level = 0.95, method = c("asymptotic", "profile"), ...)
{
  ...
  sigfit<-function(df_,df_1,i){
    ...
    df_1<-lapply(df_1,function(x)x[order(x$C),])
    #sigmoidal fit
    nlm1<-df_ %>% purrr::keep(function(x)!inherits(fitSingleSigmoid(x$C,x$I),'try-error')) #remove non-sigmoidal data
    ...
    P1<-P+
      ...
      ggplot2::labs(y = "Relative Solubility",
                    x = "Temperature")+
      #ggplot2::annotate("text", x=62, y=1, label= paste("\u03A3","RSS= ", Pred$[[i]]RSS[1] ))+
      ggplot2::annotate("text", x=62, y=0.9, label=  paste("\u0394", "AUC = ",round(Pred1[[i]]$AUC[1]-Pred[[i]]$AUC,3)))+ggplot2::annotate("text", x=62, y=0.8, label= paste("\u0394","Tm = ",round(Pred1[[i]]$Tm[1]-Pred[[i]]$Tm[1],3),"\u00B0C"))
    
    print(P1)
    #Venn Creation
    #find common IDs in trilinear vs sigmoidal
    TRvsSig<-intersect(df1$uniqueID,IID$uniqueID)
